...
def main(opt_path = None, electrode = None, sorter_fold = None, mode='few_shot',model=None):
   ...
   run_sim_sorter(generate.rec_path, restored_file, sorter_list, method_list, study_folder, generate.bad_chans, factor=generate.factor)
...
if __name__ == '__main__':
   ...
   sorter_fold_list = ['rec_np_rd_nm_p1_5_kd']
   electrode = 'np128' # sqmea: 100 / np128: 128
   
   for factor in factor_list:
      for sorter_fold in sorter_fold_list:
         opt_path = 'options_linux_list/{}/{}/factor{}.yml'.format(model, sorter_fold, factor) # zero-shot / few-shot
         main(opt_path=opt_path, electrode=electrode, sorter_fold=sorter_fold, mode=mode, model=model)
