...
from basicsr.utils.logger import initialized_logger
...
def read_options(opt_path,is_train=True):
    opt = parse(opt_path, is_train=is_train)
    launcher = 'none'
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            logger.info(
                'Training statistics:'
                f'\n\tNumber of train signals: {len(train_set)}'
            ...
            val_loader = create_dataloader(
                ...
                seed=opt['manual_seed'])
            logger.info(
                f'Number of val signals in {dataset_opt["name"]}: '
                f'{len(val_set)}')
        else:
...
def train_model(opt_path=None):
    # parse options, set distributed setting, set ramdom seed
    # opt = parse_options(is_train=True)
    # opt = read_options(opt_path='//Reconstrcution_SpkRes.yml', is_train=True)
    ...
    # automatic resume ..
    state_folder_path = 'experiments/{}/{}/training_states/'.format(opt['name'], opt['exp_name'])
    import os
    # state_folder_path = os.path.join(opt['settings']['experiments_path'],
    #                                  'experiments/{}/training_states'.format(opt['name']))
    ...
    while current_iter <= total_iters:
        ...
        # end training loop
        if early_stop_cfg is not None and no_improve_count >= patience:
    ...
    if opt.get('val') is not None:
        model.validation(val_loader, current_iter, tb_logger,
                         opt['val']['save_img'], rgb2bgr=False, use_image=False)
    if tb_logger:
        tb_logger.close()
