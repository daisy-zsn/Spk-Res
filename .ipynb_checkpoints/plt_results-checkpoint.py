...
def generate_method_comparison_tables(edf_raw, factors, electrode):
    ...
    metrics_list = [
        # ('num_gt', 'GT', 0),
        ('method', 'Method', None),
        ('num_sorter', 'Detected', 0),
        ('num_well_detected', 'Well-Detected', 0),
        ('num_false_positive', 'False Positive', 0),
        ('num_redundant', 'Redundant', 0),
        ('num_overmerged', 'Overmerged', 0),
        ('num_bad', 'Bad', 0),
        ('recall', 'Recall', 3),
    ...
    for cfg_i in config_list:
        ...
        for sorter_name in sorted(df_cfg['sorter_name'].dropna().unique()):
            ...
            for factor in factors:
                 for col_key, col_label, precision in metrics_list: 
                    for method in METHODS:
                        row = {'Factor': factor, 'Method': method, 'Metric': col_label}  
                        sub = edf_raw[(edf_raw['factor'] == factor) &
                                    (edf_raw['method'] == method) &
                                    (edf_raw['sorter_name'] == sorter_name)]
                        row = 
                        # if len(sub) > 0:
                        #     val = sub[col_key].values[0]
                        #     if precision == 0:
                        #         row[METHOD_LABELS[method]] = int(val)
                        #     else:
                        #         row[METHOD_LABELS[method]] = round(float(val), precision)
                        # else:
                        #     row[METHOD_LABELS[method]] = '---'
                        rows.append(sub)
                    
            df_out = pd.DataFrame(rows)
            # cols = ['Factor', 'Sorter', 'Metric'] + [METHOD_LABELS[m] for m in METHODS]
            cols = ['Factor', 'Metric']
            df_out = df_out[cols]

            csv_path = f'tables/{electrode}/{cfg_i}/{sorter_name}.csv'
            df_out.to_csv(csv_path, index=False)
            print(f'[Table Saved] {csv_path}')

            # ---- LaTeX 三线表 ----
            generate_method_comparison_latex(df_out, electrode)
    ...
    # # ---- 构建 pivot 表格: Factor | Sorter | Metric | Kriging | Remove | EDSR | SpkRes ----
    # rows = []
    # for factor in factors:
    #     for sorter_name in sorted(edf_raw['sorter_name'].dropna().unique()):
    #         for col_key, col_label, precision in metrics_list:
    #             row = {'Factor': factor, 'Sorter': sorter_name, 'Metric': col_label}
    #             for method in METHODS:
    #                 sub = edf_raw[(edf_raw['factor'] == factor) &
    #                               (edf_raw['method'] == method) &
    #                               (edf_raw['sorter_name'] == sorter_name)]
    ...
    # csv_path = f'tables/method_comparison_{electrode}_per_sorter.csv'
    # df_out.to_csv(csv_path, index=False)
    # print(f'[Table Saved] {csv_path}')
...
def generate_method_comparison_latex(df, electrode):
    """从 pivot 表格生成 LaTeX 三线表，聚焦方法间对比。"""
    lines = []
    n_cols = len(df.columns)
    col_str = 'l l l' + ' c' * (n_cols - 3)

    lines.append('\\begin{table}[htbp]')
    lines.append('  \\centering')
    lines.append(f'  \\caption{{Method Comparison — {ELECTRODE_LABELS.get(electrode, electrode)} (per-sorter)}}')
    lines.append(f'  \\label{{tab:method_comparison_{electrode}}}')
    ...
    # Rows — group by Factor + Sorter, add midrule between groups
    prev_key = None
    for i, (_, row) in enumerate(df.iterrows()):
        key = (row['Factor'], row['Sorter'])
    ...
    tex_path = f'tables_latex/method_comparison_{electrode}_per_sorter.tex'
...
def plot_summary_figure(df, electrode='np128', save_path=None):
    ...
    for ni, nm in enumerate(['w/o Normalization', 'w/ Normalization']):
        ...
        ax5.plot(pos, ndf['recall'], marker=mk, linestyle='-', color=c,
                 linewidth=1, markersize=2, label=f'SpkRes {nm} Recall')
...
def plot_zero_shot_vs_few_shot(df_all, save_path=None):
    ...
    pairings = [
        ...
        # EDSR transfer
        ('zs_sq100_rd_nm', 'edsr', 'Zero-shot (EDSR)'),
        ('zs_sq100_rd_nm_gcl', 'edsr', 'Zero-shot GCL (EDSR)'),
